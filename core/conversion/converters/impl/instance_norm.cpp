...
auto instance_norm_registrations TRTORCH_UNUSED = RegisterNodeConversionPatterns().pattern({
    ...
    [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
      ...
      torch::Tensor scale, bias, mean, var;

      if (ctx->input_is_dynamic) {
        scale = args[1].unwrapToTensor();
        bias = args[2].unwrapToTensor();
        mean = args[3].unwrapToTensor();
        var = args[4].unwrapToTensor();
      } else {
        scale = args[1].unwrapToTensor(at::full({shape}, 1, {options}));
        bias = args[2].unwrapToTensor(at::full({shape}, 1, {options}));
